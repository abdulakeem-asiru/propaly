...
const StepThree = ({
  ...
  const isDisabled = !selectedSource || isLoading;
  return (
    <div>
      <motion.div
        key="step3"
      ...
      >
        ...
        <div className="grid grid-cols-1 gap-3">
          {sources.map((source, index) => (
            <motion.button
              ...
              onClick={() => form.setValue("source", source)}
              className={`p-4 rounded-lg border-2 transition-all text-left ${
                form.getValues("source") === source
        ...
        <div className="mt-8 flex justify-between">
          ...
          <button
            type="submit"
            disabled={isDisabled}
            className={`bg-(--primary-color) rounded-full text-white px-8 h-12 transition
    ${isDisabled ? "opacity-70 cursor-not-allowed" : "hover:bg-[#e55d1f]"}
  `}
          >
